...
public:
    ...
    int lcsTab(string& text1, string& text2)
    {
        int m = text1.size();
        int n = text2.size();
        // State definition : dp[i][j] -> LCS of text1 till length i and text2 of length j
        ...
        for (int i = 1; i <= m; i++) {
            for (int j = 1; j <= n; j++) {
                if (text1[i - 1] == text2[j - 1])
                    ...
                    dp[i][j] = max(dp[i - 1][j], dp[i][j - 1]); // max of LCS without an alphabet in text1 or without an alphabet in text2
            }
        }
